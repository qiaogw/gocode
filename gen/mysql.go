...
// GetDB 获取数据库的所有数据库名
func (s *autoCodeMysql) GetDB() (data []models.Db, err error) {
	var entities []models.Db
...
func (s *autoCodeMysql) GetTables(dbName string) (data []models.Table, err error) {
	var entities []models.Table
	sql := `select table_name as table_name from information_schema.tables where table_schema = ?`
	err = global.GenDB.Raw(sql, dbName).Scan(&entities).Error
	return entities, err
}
...
func (s *autoCodeMysql) GetColumn(tableName string) (data []models.Column, err error) {
	var entities []models.Column
	sql := `
	...
	WHERE table_name = ?
	`
	err = global.GenDB.Raw(sql, tableName).Scan(&entities).Error
	return entities, err
}
