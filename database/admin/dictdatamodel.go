// package model 自动生成模板DictData(字典数据)
package admin
...
func NewDictDataModel(conn sqlx.SqlConn, c cache.CacheConf, gormX *gorm.DB) DictDataModel {
	return &customDictDataModel{
		defaultDictDataModel: newDictDataModel(conn, c, gormX),
	}
}
