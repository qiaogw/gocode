...
type (
	...
	MenuModel interface {
		...
		FindByRole(ctx context.Context, roleId int64) ([]*Menu, error)
	}
...
func NewMenuModel(conn sqlx.SqlConn, c cache.CacheConf, gormX *gorm.DB) MenuModel {
	return &customMenuModel{
		defaultMenuModel: newMenuModel(conn, c, gormX),
	}
}
...
func (m *defaultMenuModel) FindByRole(ctx context.Context, roleId int64) ([]*Menu, error) {
	var role Role
	var list []*Menu
	l := make([]*Menu, 0)
