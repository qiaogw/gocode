...
const { subDefaultAvatar } = useCommon()
const $q = useQuasar()
...
const avatarSrc = computed(() => {
  const cookieAvatar = $q.cookies.get('sub-avatar')
  if (loginUser.value) {
    // 登录用户的头像
    if (cookieAvatar) {
      // 如果Cookies里拿到头像，就显示
      if (cookieAvatar.substring(0, 4) === 'http') {
        // 头像为链接
        return cookieAvatar
      } else if (cookieAvatar.substring(0, 11) === 'sub-upload:') {
        // 头像为上传
    ...
    // 非登录上传，头像为链接
    return src.value
  } else if (src.value.substring(0, 11) === 'sub-upload:') {
    // 非登录用户，头像为上传
